...
static const u8 sText_MessageColor[]    = _("MESSAGE FRAME");
static const u8 sText_Font[]            = _("FONT");
static const u8 sText_UnitSystem[]      = _("UNIT SYSTEM");
static const u8 sText_Clock[]           = _("TIME SYSTEM");
...
static const u8 sText_OptionRTC[]           = _("RTC");
static const u8 sText_OptionAsynchronus[]   = _("ASYNC");
...
static const sItemFunctionsGame[MENUITEM_COUNT] =
{
    ...
    [MENUITEM_CLOCK]         = {DrawChoices_ClockMode,        ProcessInput_Options_Two},
...
{
    u8 windowColor = gSaveBlock2Ptr->optionsMessageFrame;
    u32 i, taskId;
    switch (gMain.state)
    {
    default:
    ...
    case 6:
        ...
        sOptions->sel[MENUITEM_MESSAGE_COLOR] = gSaveBlock2Ptr->optionsMessageFrame;
        sOptions->sel[MENUITEM_CLOCK]         = gSaveBlock2Ptr->optionsClockMode;
...
static void Task_OptionMenuSave(u8 taskId)
{
    ...
    gSaveBlock2Ptr->optionsMessageFrame    = sOptions->sel[MENUITEM_MESSAGE_COLOR];
    gSaveBlock2Ptr->optionsClockMode     = sOptions->sel[MENUITEM_CLOCK];
...
static int ProcessInput_Options_MessageColor(int selection)
{
    if (JOY_NEW(DPAD_RIGHT))
    {
        if (++selection > (NUM_OF_MESSAGE_BOXES - 1))
            selection = 0;
    }
    if (JOY_NEW(DPAD_LEFT))
    {
...
static void DrawChoices_MessageColor(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_MESSAGE_COLOR);
    u8 text[16];
    u8 n = selection + 1;
    u16 i;
...
static void DrawChoices_ClockMode(int selection, int y)
{
    ...
    DrawOptionMenuChoice(sText_OptionAsynchronus, 104, y, styles[0], active);
    DrawOptionMenuChoice(sText_OptionRTC, GetStringRightAlignXOffset(1, sText_OptionOff, 198), y, styles[1], active);
}
...
#define WINDOW_NPC_OFFSET (TILE_TOP_CORNER_L + 18)
...
{
    // Description window
    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 0,                    0, 14,  1, 1, 9);
    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 1,                    1, 14,  1, 1, 9);
    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 2,                    2, 14, 26, 1, 9);
    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 3,                   28, 14,  1, 1, 9);
    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 4,                   29, 14,  1, 1, 9);

    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 5,                     0, 15, 1, 1, 9);
    FillBgTilemapBufferRect(1, WINDOW_NPC_OFFSET + 6,                     1, 15, 1, 1, 9);
    ...
    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET) + 8,    28, 18, 1, 1, 9);
    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET) + 9,    29, 18, 1, 1, 9);

    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET + 0),    0, 19,  1, 1, 9);
    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET + 1),    1, 19,  1, 1, 9);
    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET + 2),    2, 19, 26, 1, 9);
    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET + 3),   28, 19,  1, 1, 9);
    FillBgTilemapBufferRect(1, BG_TILE_V_FLIP(WINDOW_NPC_OFFSET + 4),   29, 19,  1, 1, 9);
