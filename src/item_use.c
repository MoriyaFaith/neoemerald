...
void ItemUseOutOfBattle_TMHM(u8 taskId)
{
    if (gSpecialVar_ItemId >= ITEM_HM01_CUT)
        DisplayItemMessage(taskId, gText_BootedUpHM, BootUpSoundTMHM); // HM
    else
        DisplayItemMessage(taskId, gText_BootedUpTM, BootUpSoundTMHM); // TM
}
...
static void Task_ShowTMHMContainedMessage(u8 taskId)
{
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        DisplayItemMessage(taskId, gStringVar4, UseTMHMYesNo);
    }
}
...
void ItemUseOutOfBattle_Repel(u8 taskId)
{
    if (VarGet(VAR_REPEL_STEP_COUNT) == 0)
        ...
        DisplayItemMessage(taskId, gText_RepelEffectsLingered, CloseItemMessage);
    else
...
void ItemUseInBattle_PokeBall(u8 taskId)
{
    ...
    if (FlagGet(FLAG_SYS_NO_CATCHING)){ //DEBUG
        ...
        DisplayItemMessage(taskId, sText_BallsCannotBeUsed, CloseItemMessage);
        return;
    }
    #endif
    ...
    {
        DisplayItemMessage(taskId, gText_BoxFull, CloseItemMessage);
    }
    else
...
static void Task_UseStatIncreaseItem(u8 taskId)
{
    if(++gTasks[taskId].data[8] > 7)
    {
        ...
        RemoveBagItem(gSpecialVar_ItemId, 1);
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, UseStatIncreaseItem(gSpecialVar_ItemId), Task_CloseStatIncreaseMessage);
        else
...
void ItemUseInBattle_StatIncrease(u8 taskId)
{
    ...
    {
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, gText_WontHaveEffect, CloseItemMessage);
        else
...
void ItemUseInBattle_Escape(u8 taskId)
{
    ...
    if((gBattleTypeFlags & BATTLE_TYPE_TRAINER) == FALSE)
    {
        RemoveUsedItem();
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, gStringVar4, Task_FadeAndCloseBagMenu);
        else
