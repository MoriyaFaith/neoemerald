...
static const struct MenuAction sItemPcSubmenuOptions[] = {
    ...
    { gPCText_Give,     {.void_u8 = Task_ItemPcGive} },
...
static void ItemPc_BuildListMenuTemplate(void)
{
    ...
    {
        sListMenuItems[i].name = ItemId_GetName(gSaveBlock1Ptr->pcItems[i].itemId);
...
static void ItemPc_MoveCursorFunc(s32 itemIndex, bool8 onInit, struct ListMenu * list)
{
    ...
    {
        //HideBagItemIconSprite(sStateDataPtr->itemMenuIconSlot ^ 1);
        RemoveBagItemIconSprite(sStateDataPtr->itemMenuIconSlot);
        if (itemIndex != -2)
        {
        ...
        {
            AddBagItemIconSprite(-1, sStateDataPtr->itemMenuIconSlot);
...
static void ItemPc_PrintWithdrawItem(void)
{
    ItemPc_AddTextPrinterParameterized(2, 0, gText_WithdrawItem2, 0, 1, 0, 1, 0, 0);
}
...
static void Task_ItemPcSubmenuInit(u8 taskId)
{
    ...
    PrintMenuActionTextsAtPos(4, 2, 8, 2, GetFontAttribute(2, FONTATTR_MAX_LETTER_HEIGHT) + 2, 3, sItemPcSubmenuOptions);
...
static void ItemPc_DoWithdraw(u8 taskId)
{
    ...
    if (AddBagItem(itemId, data[8]) == TRUE)
    {
        ...
        StringExpandPlaceholders(gStringVar4, gText_WithdrawXItems);
    ...
    {
        windowId = ItemPc_GetOrCreateSubwindow(2);
        AddTextPrinterParameterized(windowId, 2, gText_NoRoomInBag, 0, 2, 0, NULL);
...
static void ItemPc_WithdrawMultipleInitWindow(u16 slotId)
{
    ...
    CopyItemName(itemId, gStringVar1);
    StringExpandPlaceholders(gStringVar4, gText_WithdrawHowManyItems);
...
static void Task_ItemPcGive(u8 taskId)
{
    if (CalculatePlayerPartyCount() == 0)
    {
        ...
        ClearWindowTilemap(4);
        PutWindowTilemap(0);
        ItemPc_PrintOnWindow5WithContinueTask(taskId, gText_NoPokemon, Task_ItemPcWaitButtonAndExitSubmenu);
    }
    else
    {
        sStateDataPtr->savedCallback = CB2_PartyMenuFromItemPC;
...
static void ItemPc_InitWindows(void)
{
    ...
    LoadThinWindowBorderGfx(0, 0x3A3, 0xC0);
...
static void ItemPc_PrintOnWindow5WithContinueTask(u8 taskId, const u8 * str, TaskFunc taskFunc)
{
    DisplayMessageAndContinueTask(taskId, 5, 0x3AC, 0x0B, 2, GetPlayerTextSpeedDelay(), str, taskFunc);
    ScheduleBgCopyTilemapToVram(0);
}
